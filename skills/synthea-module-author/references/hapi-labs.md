...
Use `-d` to load the modules from `iris-fhir-converter-demo`. Always `rm output/fhir/raw/*.json` first — Synthea appends, not overwrites.

```bash
cd ~/Developer/misc/synthea
rm -f output/fhir/raw/*.json
...
Output: `output/fhir/raw/` — one FHIR Bundle JSON per patient.
