...
Use `-d` to load the modules from `iris-fhir-converter-demo`. Always `rm output/raw/fhir/*.json` first — Synthea appends, not overwrites.

```bash
cd ~/Developer/misc/synthea
rm -f output/raw/fhir/*.json
...
Output: `output/raw/fhir/` — one FHIR Bundle JSON per patient.
